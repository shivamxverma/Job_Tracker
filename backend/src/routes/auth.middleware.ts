...
export const requireAuth = (req: Request, res: Response, next: NextFunction): void => {
  // Always permit CORS preflight OPTIONS requests without auth checks
  ...
  if (!apiKey || apiKey !== expectedKey) {
    console.warn(`[Auth Middleware] Unauthorized access attempt from IP: ${req.ip} on ${req.method} ${req.url}`);
    res.status(401).json({
      success: false,
